  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        ...
        >
          <img
            className="absolute inset-0 h-[120%] w-[120%] object-cover transition-opacity duration-500 ease-in-out"
        ...
        >
          ...
          <div className="flex justify-center">
            <button className="mt-6 flex h-12 w-fit items-center justify-center rounded-2xl border border-transparent bg-white px-4 py-2 text-xs text-black shadow transition duration-200 hover:shadow-lg md:text-sm">
              {button}
            </button>
          </div>
        </article>
...
const CarouselControl = ({ type, title, handleClick }: CarouselControlProps) => {
  return (
    <button
      className={`mx-2 flex h-10 w-10 items-center justify-center rounded-full border-3 border-transparent bg-neutral-200 transition duration-200 hover:-translate-y-0.5 focus:outline-none active:translate-y-0.5 dark:bg-neutral-800 ${
...
export function AlphaCarousel({ slides }: CarouselProps) {
